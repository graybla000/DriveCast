...
import { Heart, Play, Clock, MapPin, Star } from "lucide-react";
    ...
    <div className="flex items-center gap-3 p-2.5 rounded-2xl glass hairline">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-1 text-[11px] text-muted-foreground font-semibold">
          <span className="flex items-center gap-0.5"><Star size={10} className="text-gold fill-gold" />{item.rating.toFixed(1)}</span>
          <span className="flex items-center gap-0.5"><Clock size={10} />{item.duration}m</span>
          {item.distance != null && <span className="flex items-center gap-0.5"><MapPin size={10} />{item.distance}mi</span>}
        </div>
      </div>
